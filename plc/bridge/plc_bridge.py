...
class PLCBridge:
    def __init__(self, plc_host: str, plc_port: int, redis_host: str, redis_port: int):
        ...
        logger.info(
            f"🔧 Initialized PLC Bridge - PLC: {plc_host}:{plc_port}, Redis: {redis_host}:{redis_port}")
    ...
    def connect(self) -> bool:
        ...
        try:
            if self.modbus_client.connect():
                self.connected = True
                logger.info(
                    f"🔌 Connected to PLC at {self.plc_host}:{self.plc_port}")
                return True
            else:
                logger.error(
                    f"❌ Failed to connect to PLC at {self.plc_host}:{self.plc_port}")
                return False
    ...
    def _write_signal(self, signal: ModbusSignal, value: Union[bool, int]) -> bool:
        ...
        try:
            if signal.type == "Digital Output Coil":
                ...
                logger.warning(
                    f"⚠️ Cannot write to read-only signal type: {signal.type}")
                return False
        ...
        while self.running:
            try:
                ...
                if sleep_time > 0:
                    ...
                    logger.warning(
                        f"⚠️ Polling cycle taking too long: {elapsed:.3f}s")
